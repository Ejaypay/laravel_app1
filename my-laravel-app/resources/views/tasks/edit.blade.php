    ...
    <div class="mx-auto max-w-2xl rounded-xl bg-white p-8 shadow">
        ...
        <form action="{{ route('tasks.update', $task) }}" method="POST" class="mt-6 space-y-4">
            ...
            <div>
                ...
                <input type="text" name="title" value="{{ old('title', $task->title) }}" class="w-full rounded border px-4 py-2">
            </div>

            <div>
                <label class="mb-2 block font-medium">Description</label>
                <textarea name="description" class="w-full rounded border px-4 py-2">{{ old('description', $task->description) }}</textarea>
            </div>
            ...
            <button type="submit" class="rounded bg-blue-600 px-4 py-2 text-white">Update Task</button>
        </form>
    </div>
@endsection